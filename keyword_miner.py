...
def get_news_articles():
    urls = ["https://news.google.com/rss/headlines/section/topic/BUSINESS?hl=en-US&gl=US&ceid=US:en"]
...
def get_golden_keyword_and_source():
    ...
    if not articles:
        articles = [{"title": "Real Estate & Housing Economics Major Industry Update", "description": "Key trends, metrics, and structural changes observed in the sector.", "date": "2026"}]

    headlines = [a["title"] for a in articles]
    prompt = f"""
    Review the following latest headlines in Real Estate & Housing Economics:
    ...
    try:
        ...
        data = json.loads(res)
        kw = data.get("golden_keyword", "Real Estate & Housing Economics Trends")
    except Exception as e:
        kw = "Real Estate & Housing Economics Strategic Analysis"
